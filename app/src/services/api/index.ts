 ...
 */
function transformSupabasePhoto(supabasePhoto: any): Photo {
  // v6.0: 优先使用 OSS URL，回退到 Supabase Storage
  let photoUrl = supabasePhoto.oss_url || 
    supabase.storage
      .from('photos-original')
      .getPublicUrl(supabasePhoto.storage_path).data.publicUrl;
...
export const apiService = {
  ...
  async uploadPhoto(params: {
    ...
    filename: string;
    tags?: string[];
  }): Promise<ApiResponse<{ photoId: string; faces: any[] }>> {
    if (useMSW) {
  ...
  async getAlbums(): Promise<Album[]> {
    if (useMSW) {
      ...
      const albums = (data || []).map(item => {
        ...
        const coverPhotoUrl = item.cover_photo?.oss_thumbnail_url 
      ...
      if (albumsNeedingCover.length > 0) {
        ...
        const allPhotoIds = [...new Set(
          albumsNeedingCover.flatMap(a => a.photoIds.slice(0, 5))
        )];
        ...
        if (photosData && photosData.length > 0) {
          // 创建照片 ID 到 URL 的映射
          const photoUrlMap = new Map(photosData.map(p => [
            p.id, 
            p.oss_thumbnail_url || p.oss_compressed_url || p.oss_url
          ]));
  ...
  async getAlbumById(albumId: string): Promise<Album | null> {
    if (useMSW) {
      ...
      // 提取封面照片 URL（优先级：thumbnail > compressed > original）
      let coverPhotoUrl = data.cover_photo?.oss_thumbnail_url 
      ...
      if (!coverPhotoUrl && photoIds.length > 0) {
        ...
        if (photoData) {
          coverPhotoUrl = photoData.oss_thumbnail_url 
            || photoData.oss_compressed_url 
            || photoData.oss_url;
        }
      }
  ...
  async getPersons(): Promise<Person[]> {
    if (useMSW) {
      ...
      return memoryCache.getOrFetch('api:persons', async () => {

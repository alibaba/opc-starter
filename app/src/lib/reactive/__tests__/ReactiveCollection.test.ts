...
type MockPhoto = Pick<Photo, 'id' | 'tags' | 'participants'> & {
  title: string
  url: string
  albumId?: string
  createdAt?: Date
    ...
    query: vi.fn(async (options) => {
      ...
      if (options.filter && typeof options.filter === 'object') {
        results = results.filter(item => 
          Object.entries(options.filter as object).every(([k, v]) => 
...
const createMockRemoteAdapter = (): RemoteAdapter<MockPhoto> & {
  ...
  return {
    ...
    subscribe: vi.fn((callback) => {
      ...
      return () => { subscribeCallback = undefined }
    }),
...
describe('ReactiveCollection', () => {
  ...
  describe('find() - 响应式查询', () => {
    ...
    it('应该支持过滤条件', async () => {
      await collection.insert({ title: 'Photo A', url: 'a.jpg', albumId: 'album-1', tags: [], participants: [] })
      await collection.insert({ title: 'Photo B', url: 'b.jpg', albumId: 'album-2', tags: [], participants: [] })
  ...
  describe('insert() - 乐观插入', () => {
    it('应该立即更新本地数据（乐观更新）', async () => {
      ...
      const promise = collection.insert({ title: 'New Photo', url: 'new.jpg', tags: [], participants: [] })
    ...
    it('应该用服务端 ID 替换临时 ID', async () => {
      ...
      const result = await collection.insert({ title: 'New Photo', url: 'new.jpg', tags: [], participants: [] })
  ...
  describe('update() - 乐观更新', () => {
    ...
    it('应该在失败时回滚到原始值', async () => {
      ...
      await expect(
        collection.update(inserted.id, { title: 'Updated' })
  ...
  describe('Realtime Sync - 实时同步', () => {
    it('应该处理远程 INSERT 事件', async () => {
      ...
      mockRemoteAdapter.simulateChange({
        type: 'INSERT',
        doc: { id: 'remote-new', title: 'Remote Photo', url: 'remote.jpg', tags: [], participants: [] },
      })

/**
 * SkillEditPage - Skill 编辑页面
 * 复用 PublishPage 的表单逻辑，预填充现有数据
 */
...
import type { SkillVisibility, SkillPlatform, Skill } from '@/types/skill'
...
export function SkillEditPage() {
  ...
  useEffect(() => {
    const loadSkill = async () => {
      ...
      try {
        const skillData = await skillService.getBySlug(slug)
        if (!skillData) {
          toast({ title: 'Skill 不存在', variant: 'error' })
          navigate('/my-skills')
          return
        }

        // 检查是否是作者
        if (skillData.author_id !== user?.id) {
          toast({ title: '无权限编辑此 Skill', variant: 'error' })
          navigate('/my-skills')
          return
        }
        ...
        setReadme(skillData.readme || '')
        setTags(skillData.tags || [])
        setSelectedPlatforms(skillData.platforms || [])
        setVisibility(skillData.visibility)

        // 计算下一个版本号（带格式校验）
        if (skillData.latest_version && /^\d+\.\d+\.\d+$/.test(skillData.latest_version)) {
          const parts = skillData.latest_version.split('.').map(Number)
          setNewVersion(`${parts[0]}.${parts[1]}.${parts[2] + 1}`)
        } else {
          setNewVersion('1.0.0') // 默认回退
        }
      } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: '保存成功',
        description: file ? 'Skill 信息和新版本已更新' : 'Skill 信息已更新',
      })
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="space-y-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle>上传新版本（可选）</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                当前版本：{skill?.latest_version || '无'}。如需发布新版本，请填写以下信息。
              </p>

              <div>
                <Label htmlFor="newVersion">新版本号</Label>
                <Input
                  id="newVersion"
                  value={newVersion}

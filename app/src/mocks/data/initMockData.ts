...
export async function initMockData(): Promise<void> {
  ...
  try {
    ...
    const existingPhotos = await photoDB.getPhotos(1, 1);

    if (existingPersons.length > 0 || existingPhotos.total > 0) {
    ...
    // 2. 插入照片数据
    console.log(`[Mock] 正在插入${MOCK_PHOTOS.length}张照片...`);
    await photoDB.addPhotos(MOCK_PHOTOS);

    console.log('[Mock] ✅ Mock数据初始化完成！');
    console.log(`[Mock] - 人员数量: ${MOCK_PERSONS.length}`);
    console.log(`[Mock] - 照片数量: ${MOCK_PHOTOS.length}`);
  } catch (error) {

...
describe('用户登录功能', function() {
  ...
  describe('登录后页面跳转验证', () => {
    it('登录成功后应该跳转到首页', () => {
      ...
      // 验证首页内容
      cy.contains('照片时光机').should('be.visible')
    })
    ...
    it('登录后应该能够访问受保护的页面', () => {
      ...
      const protectedRoutes = [
        { path: '/upload', text: '上传' },
        { path: '/timeline', text: '时间轴' },
        { path: '/albums', text: '相册' },
        { path: '/persons', text: '人员' },
      ]
  ...
  describe('导航栏状态验证', () => {
    it('登录后导航栏应该显示正确的菜单项', () => {
      ...
      // 验证主要导航项（根据实际项目调整选择器）
      const navItems = ['首页', '上传', '时间轴', '相册', '人员']
      
      navItems.forEach((item) => {
        // 使用更宽松的匹配，因为导航项可能在侧边栏或顶部栏
    ...
    it('登录后应该能够通过导航栏切换页面', () => {
      ...
      // 点击时间轴
      cy.contains('时间轴').click()
      cy.url().should('include', '/timeline')
      
      // 点击相册
      cy.contains('相册').click()
      cy.url().should('include', '/albums')
      
      // 点击人员
      cy.contains('人员').click()
